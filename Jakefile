...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "IsCssBasedIssue.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("IsCssBasedIssue");
    task.setIdentifier("com.yourcompany.IsCssBasedIssue");
    task.setVersion("1.0");
    task.setAuthor("Army of Me, Inc.");
    task.setEmail("feedback @nospam@ yourcompany.com");
    task.setSummary("IsCssBasedIssue");
